...
export function RolesCatalog() {
  ...
  const selected =
    roles.find(role => role.key === selectedKey) ??
    filtered[0] ??
    roles[0] ??
    null
  ...
  const system = filtered.filter(item => item.isSystem)
  const custom = filtered.filter(item => !item.isSystem)
  ...
  return (
    <DashboardContentShell
      title="Roles"
      subTitle="Bundles of permissions you assign to staff."
      actions={
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            variant={
                              selected.isSystem ? 'outline' : 'secondary'
                            }
                          >
